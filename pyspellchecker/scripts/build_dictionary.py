    ...
    with load_file(filepath, 'utf-8') as fobj:
        for line in fobj:
            ...
            words = [word[0].lower() for word in tagged_sent if
                    #  word[0] and not word[1] == "NNP" and 
                     word[0][0].isalpha() and word[0][-1].isalpha()]
...
def clean_hebrew(word_frequency, filepath_exclude, filepath_include):
    letters = set('אבגדהוזחטיכךלמםנןסעפףצץקרשת"')
    ...
    # Remove words with final letters in the middle of the word (ם,ן,ך,ץ,ף)
    end_letters = list()
    for key in word_frequency:
        if key.find('ם') != -1 and (not key.endswith('ם')) \
                or key.find('ן') != -1 and (not key.endswith('ן')) \
                or key.find('ץ') != -1 and (not key.endswith('ץ')) \
                or key.find('ך') != -1 and (not key.endswith('ך')) \
                or key.find('ף') != -1 and (not key.endswith('ף')):
            end_letters.append(key)
    for misfit in end_letters:
        word_frequency.pop(misfit)

    # Removing words that the last letter should be final
    not_end_letter = list()
    for key in word_frequency:
        if key.endswith('מ') or key.endswith('נ') or key.endswith('צ') or key.endswith('כ') or key.endswith('פ'):
            not_end_letter.append(key)
    for misfit in not_end_letter:
        word_frequency.pop(misfit)

    # remove words with quotes that might be interpreted as acronyms
    acronyms = []
    for key in word_frequency:
        if key.find('"') != -1:
            if '"' != key[-2]:
                acronyms.append(key)
    for misfit in acronyms:
        word_frequency.pop(misfit)
...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Build a new dictionary (word frequency) using the OpenSubtitles2018 project')
    parser.add_argument("-l", "--language", required=True, help="The language being built",
                        choices=['en', 'es', 'de', 'fr', 'pt', 'ru', 'he'])
    parser.add_argument("-f", "--file-path",
                        help="The path to the downloaded text file OR the saved word frequency json")
    parser.add_argument("-p", "--parse-input", action="store_true",
                        help="Add this if providing a text file to be parsed")
    parser.add_argument("-m", "--misfit-file", action="store_true",
...
if __name__ == '__main__':
    ...
    if args.language == "en":
        ...
        word_frequency = clean_hebrew(word_frequency, exclude_filepath, include_filepath)
